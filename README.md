...
1. 手动粘贴/上传 HTML，或由 Android 教务系统 WebView 在用户点击识别时临时取得当前页面 DOM。Android 导入会使用当前已登录会话及选中的学年、学期 ID 请求校历，自动填入开学日期；校历响应只在内存中解析，不展示或保存，自动获取失败时可手动填写日期。
